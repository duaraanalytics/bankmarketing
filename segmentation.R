...
raw.data <- read.xlsx("Online Retail.xlsx", sheet = 1, startRow = 1, colNames = TRUE)

# Prepare data
data <- raw.data #  raw.data as a backup
str(data)
...
#restrict the date range to one full year.
range(data$InvoiceDate)
data <- subset(data, InvoiceDate >= "2010-12-09")
range(data$InvoiceDate)

#restrict the data to one geographic unit.
table(data$Country)
...
scatter.3 <- ggsave("download3.png")
